...
// Convenience for route handlers: keys by account when the caller is signed
// in, otherwise by IP. Returns a 429 response to return, or null to continue.
export function limitRequest({ request, user = null, bucket, limit, windowMs, hint }) {
    const key = `${bucket}:${user ? user.id : getClientIp(request)}`;
    const quota = consumeRateLimit(key, limit, windowMs);
    return quota.allowed ? null : rateLimitResponse({ resetAt: quota.resetAt, hint });
}
