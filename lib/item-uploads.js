// Creates custom items from in-game uploads (the mod's /sts upload_item command
...
import { hasCustomItemName, saveCustomItem, BUILD_NAME_MAX } from './sts-builds.js';
import { buildCustomItemVocab, coerceBaseItem, coerceItemType, sanitizeItemStats } from './custom-item-vocab.js';
...
function sanitizeBaseItem(baseItem) {
    ...
    const cleaned = baseItem.trim().split(':').pop().replaceAll('_', ' ').replace(/\s+/g, ' ').trim();
...
export function createUploadedCustomItems({ userId, authorName, authorAvatar, items, itemData }) {
    ...
    const vocab = buildCustomItemVocab(itemData);
    const created = [];
    const skipped = [];
    for (const raw of (items || []).slice(0, 50)) {
        const name = typeof raw?.name === 'string' ? raw.name.trim().slice(0, BUILD_NAME_MAX) : '';
        if (!name) {
        ...
        const baseItem = coerceBaseItem(sanitizeBaseItem(raw.baseItem), vocab);
        const textureToken = resolveTextureToken(name, raw.textureName);
        const { stats: loreStats, colors } = mapLoreToStats(raw.lore || [], index, { enchants: raw.enchants || {} });
        // Explicit stats (if the mod ever sends them) win over lore parsing.
        const merged = { ...loreStats };
        ...
        // Known stat keys only, with bounded count and numeric range.
        const stats = sanitizeItemStats(merged, vocab);
        ...
        const item = saveCustomItem({
            userId,
            name,
            type: coerceItemType(raw.type, vocab),
            textureToken,
            textureName: name,
            stats,
            statColors: colors,
