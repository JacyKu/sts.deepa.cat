...
export function saveBuild({ state, userId, name, notes, summary, source = null }) {
    ...
    const existing = userId
        ? db
              .prepare('SELECT id, name, notes FROM builds WHERE state = ? AND user_id = ? LIMIT 1')
              .get(canonical, userId)
        : db.prepare('SELECT id, name, notes FROM builds WHERE state = ? AND user_id IS NULL LIMIT 1').get(canonical);
    if (existing) {
...
export function getBuild(id) {
    ...
    const row = db
        .prepare(
            'SELECT id, token, user_id, state, name, notes, created_at, updated_at, revision, is_public, anonymous, author_name, author_avatar, class_name, spec, region, power, has_charms, masterwork_count, cz_tree, skills_json, items_json, publicized_at FROM builds WHERE id = ? COLLATE NOCASE'
        )
        .get(id);
    if (!row) return null;
