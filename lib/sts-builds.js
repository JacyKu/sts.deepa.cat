    ...
    if (basic && typeof basic === 'object') {
        const sortedBasic = Object.fromEntries(
            Object.entries(basic)
                ...
                .map(([slot, v]) => [slot, { name: v.name, level: Math.max(1, Math.min(4, Number(v.level) || 1)) }])
                .sort(([a], [b]) => (a < b ? -1 : a > b ? 1 : 0))
        );
...
// live in the database and are served from /api/v1/account/avatars/<id>.
// ---------------------------------------------------------------------------
...
export function uploadedAvatarUrl(id) {
    return id ? `/api/v1/account/avatars/${id}` : null;
}
