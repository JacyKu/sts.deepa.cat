...
// editing/deleting; public items also have a public read-only share view that
// anyone with the link can see (and the public database lists them), while
// private items are visible only to their owner. Any logged-in visitor can
// duplicate a public item into their own list through the regular create
// route (which stamps the copy with the viewer's account).
// ---------------------------------------------------------------------------
...
// Items used to be public by default with no privacy switch; existing rows
// keep that behaviour (the column default is 1).
try {
    db.prepare('SELECT is_public FROM custom_items LIMIT 1').get();
} catch (e) {
    db.exec('ALTER TABLE custom_items ADD COLUMN is_public INTEGER NOT NULL DEFAULT 1');
}
...
export function saveCustomItem({
    ...
    try {
        db.prepare(
            `INSERT INTO custom_items (id, user_id, name, type, texture_token, texture_name, stats_json, stat_colors_json, author_name, author_avatar, base_item, is_public)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)`
        ).run(
            id,
            userId,
...
// stats / base item / visibility). Returns the refreshed item, or null when
// nothing was changed.
export function updateCustomItem(
    id,
    userId,
    { name, type, textureToken, textureName, stats, baseItem, isPublic } = {}
) {
...
// Public custom items database: every player's public items, newest first,
// filtered by name (LIKE) and/or exact item type. No auth needed - public
// items are shareable, so the database is just the browseable view of the
// same links. Private items never appear here.
...
// Custom item likes. Public items can be liked by any signed-in visitor
// (including their owner, same as builds); private items are not likeable
// because only their owner can see them.
db.exec(`
...
export function addCustomItemFavourite(itemId, userId) {
    ...
    const row = db.prepare('SELECT is_public FROM custom_items WHERE id = ?').get(itemId);
    if (!row || row.is_public === 0) return null;
...
// The signed-in user's liked custom items, most recently liked first. Only
// items that are still public are listed (an item made private drops out of
// other players' favourites). `q` filters by item name (LIKE,
// case-insensitive).
export function listFavouriteCustomItems({ q = null, page = 1, limit = 24, userId = null } = {}) {
    if (!userId) return { items: [], hasMore: false };
    const pageSize = Math.min(50, Math.max(1, Number(limit) || 24));
    const offset = (Math.max(1, Number(page) || 1) - 1) * pageSize;
    const clauses = ['cf.user_id = ?', 'ci.is_public = 1'];
    const params = [userId];
    if (q && typeof q === 'string') {
        const clean = q.trim().slice(0, 100);
        if (clean) {
...
// them) plus the custom items the build token actually references, matched by
// the token's item-name hashes. That way shared builds render other players'
// custom items for everyone without dumping the whole public database into the
// builder's item pickers.
//
// Referenced items are shown to every viewer, signed in or not: a build keeps
// the custom items it was saved with even if their owner makes them private
// later - privacy only affects the item's own listings (database, share link,
// favourites), never the builds using it.
export function mergeReferencedCustomItems(itemData, userId, referencedHashes) {
    if (!itemData) return itemData;
    const merged = { ...itemData };
    const usedNames = new Set();
    const add = (custom, allowBaseCollision, displayOnly = false) => {
        ...
        const entry = {
            ...
            base_item: custom.baseItem,
            isCustomItem: true,
            // Signed-out viewers see the items a build was saved with, but
            // cannot equip or save them: the client disables these in the item
            // pickers and the save routes drop them.
