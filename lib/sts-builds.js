...
export function saveBuild({ state, userId, name, notes, summary, source = null }) {
    ...
    if (existing) {
        ...
        if (name !== undefined || notes !== undefined) {
            ...
            if (name !== undefined) {
                sets.push('name = ?');
                params.push(typeof name === 'string' ? name.slice(0, 30) : null);
            }
            if (notes !== undefined) {
                sets.push('notes = ?');
...
export function updateBuild(id, userId, { name, notes } = {}) {
    ...
    if (name !== undefined) {
        ...
        sets.push('name = ?');
        params.push(String(name).slice(0, 30));
    }
    if (notes !== undefined) {
...
// One saved build per name per author: true when another build owned by the
// same user already carries this name. `excludeId` lets the build being
// re-saved / renamed keep its own name. Names are stored sliced to 30 chars,
// so the comparison uses the same truncation.
export function buildNameTakenByUser(userId, name, excludeId) {
    ...
    return Boolean(
        db
            .prepare('SELECT 1 FROM builds WHERE user_id = ? AND name = ? AND id != ? LIMIT 1')
            .get(userId, clean, excludeId || '')
    );
}

// The id of the user's build carrying exactly this saved state (if any),
// used to exclude the same build when it is re-saved with the same name.
export function findBuildByState(userId, state) {
    if (!userId) return null;
    const canonical = canonicalState(state);
    if (!canonical) return null;
    const row = db.prepare('SELECT id FROM builds WHERE state = ? AND user_id = ? LIMIT 1').get(canonical, userId);
...
export function hasCustomItemName(userId, name, excludeId) {
    ...
    const row = excludeId
        ...
        : db.prepare('SELECT 1 FROM custom_items WHERE user_id = ? AND LOWER(name) = LOWER(?) LIMIT 1').get(userId, name);
    return Boolean(row);
}

// Update an owned custom item's editable fields (name / type / texture /
// stats). Returns the refreshed item, or null when nothing was changed.
export function updateCustomItem(id, userId, { name, type, textureToken, textureName, stats } = {}) {
    ...
    params.push(id, userId);
    const result = db
        .prepare(`UPDATE custom_items SET ${sets.join(', ')} WHERE id = ? AND user_id = ?`)
        .run(...params);
...
export function ensureStsUser(discordId) {
...
// --- Saved skill / delve sets ---
//
// Logged-in users can snapshot the skill portion of a build (class, spec,
// skill points, enhancements, CZ abilities - kind 'skills') or the delve
// infusions (kind 'delve') and re-apply them to other builds later. The
// payload is owned entirely by the client; the server just stores it.
db.exec(`
...
export const SKILL_SET_KINDS = ['skills', 'delve'];
...
export function saveSkillSet({ userId, kind, name, payload, className = null, spec = null }) {
    ...
    const cleanName = String(name || '').trim().slice(0, SKILL_SET_NAME_MAX);
