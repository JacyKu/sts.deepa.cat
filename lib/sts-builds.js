...
import { fnv1a32, getBuildItemSlots } from '../app/_src/utils/builder/buildUrlCodec.js';
...
// Build names are unique across the whole site (case-insensitive). The API
// checks before writing; this index makes the database enforce it too
// (races, scripts, restores). Existing duplicates are resolved by renaming
// the newer copies ("Name (2)") so the index can always be created.
try {
    const duplicates = db
        .prepare(
            `SELECT name COLLATE NOCASE AS normalized FROM builds
    ...
    if (duplicates.length > 0) {
        const taken = new Set(
            db
                .prepare("SELECT name FROM builds WHERE name IS NOT NULL AND name != ''")
                .all()
                .map((row) => row.name.toLowerCase())
        );
        const rename = db.prepare('UPDATE builds SET name = ? WHERE id = ?');
        for (const { normalized } of duplicates) {
            const rows = db
                .prepare(
    ...
    db.exec(
        `CREATE UNIQUE INDEX IF NOT EXISTS idx_builds_name ON builds (name COLLATE NOCASE)
         WHERE name IS NOT NULL AND name != ''`
    );
    db.exec('DROP INDEX IF EXISTS idx_builds_user_name');
} catch (error) {
...
// (/notify). The bot is the only writer (see /api/v1/notifications); the
    ...
    if (basic && typeof basic === 'object') {
        const sortedBasic = Object.fromEntries(
            Object.entries(basic)
                ...
                .map(([slot, v]) => [
                    slot,
                    { name: v.name, level: Math.max(1, Math.min(4, Number(v.level) || 1)) },
                ])
                .sort(([a], [b]) => (a < b ? -1 : a > b ? 1 : 0))
        );
...
export function saveBuild({ state, userId, name, notes, summary, source = null }) {
    ...
    if (existing) {
        ...
        if (name !== undefined || notes !== undefined) {
            ...
            if (name !== undefined) {
                sets.push('name = ?');
                params.push(typeof name === 'string' ? name.slice(0, BUILD_NAME_MAX) : null);
            }
            if (notes !== undefined) {
                sets.push('notes = ?');
                params.push(typeof notes === 'string' && notes ? notes.slice(0, 500) : null);
            }
...
// Build names are unique across every saved build: true when any build other
// than `excludeId` already carries this name (case-insensitive). Names are
// stored sliced to BUILD_NAME_MAX chars, so the comparison uses the same
// truncation.
export function buildNameTaken(name, excludeId) {
    if (typeof name !== 'string') return false;
    const clean = name.trim().slice(0, BUILD_NAME_MAX);
    if (!clean) return false;
    return Boolean(
        db.prepare('SELECT 1 FROM builds WHERE name = ? COLLATE NOCASE AND id != ? LIMIT 1').get(clean, excludeId || '')
    );
}

// True when a write failed because another build already carries the name
// (the unique index backstop behind buildNameTaken).
export function isDuplicateNameError(error) {
    return (
        Boolean(error) &&
        error.code === 'SQLITE_CONSTRAINT_UNIQUE' &&
        String(error.message).includes('builds.name')
    );
}
...
export function listPublicCustomItems({ q = null, type = null, types = null, page = 1, limit = 24 } = {}) {
    ...
    const rows = db
        .prepare(`SELECT * FROM custom_items${whereSql} ORDER BY created_at DESC, id DESC LIMIT ? OFFSET ?`)
...
export function addCustomItemFavourite(itemId, userId) {
    ...
    const count = db
        .prepare('SELECT COUNT(*) AS c FROM custom_item_favourites WHERE item_id = ?')
        .get(itemId).c;
    return { favourite: true, count };
}
...
export function customItemFavouriteStates(itemIds, userId = null) {
    ...
    const counts = db
        .prepare(`SELECT item_id, COUNT(*) AS c FROM custom_item_favourites WHERE item_id IN (${placeholders}) GROUP BY item_id`)
        .all(...ids);
    for (const row of counts) map[row.item_id] = { count: row.c, favourite: false };
    if (userId) {
        const mine = db
            .prepare(
                `SELECT item_id FROM custom_item_favourites WHERE user_id = ? AND item_id IN (${placeholders})`
            )
            .all(userId, ...ids);
...
export function listFavouriteCustomItems({ page = 1, limit = 24, userId = null } = {}) {
...
// The account's profile-picture preference: 'discord' (default) or
// 'minecraft' (the cached head render of their first linked UUID).
export function getAvatarSource(discordId) {
    ...
    return row && row.avatar_source === 'minecraft' ? 'minecraft' : 'discord';
}
...
export function preferredAuthorAvatar(discordId, discordAvatar = null) {
    ...
    if (getAvatarSource(discordId) === 'minecraft') {
        const link = db
...
// snapshots so existing builds and custom items follow it immediately.
export function setAvatarSource(discordId, source) {
    if (typeof discordId !== 'string' || !discordId) return null;
    const next = source === 'minecraft' ? 'minecraft' : 'discord';
    db.prepare("UPDATE users SET avatar_source = ?, updated_at = datetime('now') WHERE discord_id = ?").run(
        next,
        discordId
    );
    const avatar = preferredAuthorAvatar(discordId);
    db.prepare('UPDATE builds SET author_avatar = ? WHERE user_id = ? AND anonymous = 0').run(avatar, discordId);
    db.prepare('UPDATE custom_items SET author_avatar = ? WHERE user_id = ?').run(avatar, discordId);
    return { avatarSource: next, avatar };
}
...
export function createPendingLink(uuid) {
    ...
    db.prepare('INSERT INTO pending_links (code, uuid) VALUES (?, ?)').run(code, normalized);
    return code;
}
...
export function getPendingLink(code) {
    ...
    return (
        db
            .prepare(
                "SELECT code, uuid FROM pending_links WHERE code = ? AND created_at > datetime('now', '-15 minutes')"
            )
            .get(code) || null
    );
}
...
export function confirmPendingLink(code, discordId) {
    if (!discordId) return { error: 'expired' };
    const pending = getPendingLink(code);
    if (!pending) return { error: 'expired' };
    const existing = db.prepare('SELECT discord_id FROM minecraft_links WHERE uuid = ?').get(pending.uuid);
    if (existing && existing.discord_id !== discordId) {
        return { error: 'linked-elsewhere' };
    }
    db.prepare(
        "INSERT INTO minecraft_links (uuid, discord_id, updated_at) VALUES (?, ?, datetime('now')) ON CONFLICT(uuid) DO UPDATE SET discord_id = excluded.discord_id, updated_at = excluded.updated_at"
    ).run(pending.uuid, discordId);
...
export function getLinkByUuid(uuid) {
    ...
    if (!normalized) return null;
    const row = db
        .prepare('SELECT uuid, discord_id, created_at, updated_at FROM minecraft_links WHERE uuid = ?')
        .get(normalized);
    return row || null;
}
...
// author snapshot cleared). Favourites, custom items and Minecraft links
// are deleted.
