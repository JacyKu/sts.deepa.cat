...
export function createPendingLink(uuid, deviceToken) {
    ...
    db.prepare('INSERT INTO pending_links (code, uuid, token_hash) VALUES (?, ?, ?)').run(
        code,
        normalized,
...
export function getPendingLink(code) {
    ...
    return (
        db
            .prepare(
                "SELECT code, uuid, token_hash FROM pending_links WHERE code = ? AND created_at > datetime('now', '-15 minutes')"
            )
            .get(code) || null
    );
}
