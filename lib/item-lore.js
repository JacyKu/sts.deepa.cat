...
//
// Items uploaded from the game (the mod's /sts upload_item command, or builds that
...
export function mapLoreToStats(lines, index, { enchants } = {}) {
    const stats = {};
    const colors = {};
    // Both inputs come from the mod: bound how much work one upload can cause.
    for (const [name, level] of Object.entries(enchants || {}).slice(0, 100)) {
        const key = normalizeLabel(name);
        const value = Number(level);
        if (
            !key ||
            key.length > 128 ||
            SKIP_ENCHANTS.has(key.replace(/_/g, '')) ||
    ...
    const loreLines = Array.isArray(lines) ? lines.slice(0, 200) : [];
    for (const line of parseLoreLines(loreLines)) {
