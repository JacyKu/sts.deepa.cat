...
export async function getSession() {
    ...
    try {
        return await getIronSession(store, sessionOptions);
    } catch (e) {
        // A malformed/foreign session cookie (or one sealed with an older
        // secret) throws while unsealing. Treat just that cookie as absent so
        // public pages and API calls keep working signed-out instead of
        // 500ing; logging in replaces it with a valid one.
        const fallback = {
            get: (name) => (name === sessionOptions.cookieName ? undefined : store.get(name)),
            set: (...args) => store.set(...args),
            delete: (...args) => store.delete(...args),
        };
        try {
            return await getIronSession(fallback, sessionOptions);
        } catch (e2) {
            return { save: async () => {}, destroy: async () => {} };
        }
    }
}
