...
import { getAvatarSource, listLinksForDiscord } from './sts-builds';
...
// account's profile-picture preference (Discord avatar or the cached
// Minecraft head of their first linked UUID). The candidates are returned
// too, so the account page can preview both options.
export function resolveProfileAvatar(user) {
    ...
    const useMinecraft = Boolean(minecraft) && getAvatarSource(user.id) === 'minecraft';
    return {
        avatarUrl: useMinecraft ? minecraft : discord,
        avatarSource: useMinecraft ? 'minecraft' : 'discord',
